...
public class Bufferconcurrent {
	...
	private HashMap<String , Boolean> les_messages_envoyer_WHOSMSG =new HashMap<String,Boolean>();
	...
	public boolean envoi_WHOS = false ;
	public String name_fichier ;
	...
	public void put_msg_envoyer_whosmsg(String msg){
		this.les_messages_envoyer_WHOSMSG.put(msg, true);
	}


	public Object get_msg_envoyer_whosmsg(String msg){
		return this.les_messages_envoyer_WHOSMSG.get(msg);
	}
			...
			if (taille_idm(msg.split("\\s")[1])) {
				if (taille_msg(msg)) {
					...
					envoi = true ;
						...
						System.out.println("codage introvable ");
					}
				} else {
...
public boolean verification_codage(String msg){
	if (msg.split("\\s")[0].equals("WMSG")) {

		return taille_idm(msg.split("\\s")[1]) ;

	}else if (msg.split("\\s")[0].equals("WHOS")) {
	...
	} else if (msg.split("\\s")[0].equals("MMSG")) {
